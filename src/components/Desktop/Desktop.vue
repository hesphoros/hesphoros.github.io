<template>
  <div class=" tw-antialiased tw-w-screen tw-h-screen tw-overflow-hidden tw-flex tw-justify-center tw-items-center"
    ...
    <div ref="frontground" class="tw-absolute tw-w-full tw-h-full tw-z-10 tw-overflow-hidden"
      ...
      <div v-for="item in window_list" :key="item.uuid">
        <WindowFolder v-if="item.type === 'explorer'" :uuid='item.uuid' :startpos_x="item.spx" :startpos_y="item.spy"
          :filemap="map" :openpath="item.openpath" :zindex="item.zindex" :minimized="item.minimized" />
        ...
        <WindowMusic v-if="item.type === 'music'" :uuid='item.uuid' :startpos_x="item.spx" :startpos_y="item.spy"
          :zindex="item.zindex" :default_width="item.default_width" :default_height="item.default_height"
          :fixedsize="item.fixedsize" :minimized="item.minimized" />
        <WindowSetting v-if="item.type === 'settings'" :uuid='item.uuid' :startpos_x="item.spx" :startpos_y="item.spy"
          :zindex="item.zindex" :minimized="item.minimized" />
    ...
    <div ref="realbackground" class="tw-absolute tw-w-full tw-h-full realbackground" style="top:0;left:0"
      ...
      <div class="tw-absolute tw-w-full " @click="background_clicked"
        ...
        <ContextMenuBottomBar v-if="context_menu_show && ($store.state.current_focus === 'ContextMenuBottomBar')"
...
export default {
  ...
  data() {
    return {
      ...
      // 可用的视频壁纸列表 - 使用 GitHub Releases 托管
      videoWallpapers: [
