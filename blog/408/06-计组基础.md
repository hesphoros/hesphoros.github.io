...
【考纲内容】<br/>
（一）计算机系统层次结构<br/>
&emsp;&emsp;&emsp;计算机系统的基本组成<br/>
&emsp;&emsp;&emsp;计算机硬件的基本组成<br/>
&emsp;&emsp;&emsp;计算机软件和硬件的关系<br/>
&emsp;&emsp;&emsp;计算机系统的工作原理：“存储程序”方式；高级语言程序与机器语言程序的转换；<br/>
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;程序和指令的执行过程<br/>
（二）计算机性能指标<br/>
&emsp;&emsp;&emsp;吞吐量；响应时间；CPU时钟周期；主频；CPI；CPU执行时间；<br/>
&emsp;&emsp;&emsp;MIPS；MFLOPS；GFLOPS；TFLOPS；PFLOPS；EFLOPS；ZFLOPS<br/>
</syllabus>
...
21\. 机器A的主频为800MHz，某程序在机器A上运行需要12s。现在硬件设计人员想设计机器B，希望该程序在机器B上的运行时间能缩短为8s，使用新技术后可使机器B的主频大幅度提高，但在机器B上运行该程序所需的时钟周期数为在机器A上的1.5倍，则机器B的主频至少应为 (&emsp;)。
...
22\. (2011) 下列选项中，描述浮点数操作速度指标的是 (&emsp;)。
...
23\. (2021) 2017年公布的全球超级计算机TOP 500排名中，我国“神威·太湖之光”超级计算机蝉联第一，其浮点运算速度为93.0146 PFLOPS，说明该计算机每秒完成的浮点操作次数约为 (&emsp;)。
...
24\. 某计算机主频为1.2GHz，其指令分为4类，它们在基准程序中所占比例及CPI如下表所示。该机的MIPS数是 (&emsp;)。

<img src="./06-计组基础.assets/1-23.png" style="zoom: 60%;" />
...
25\. (2023) 若机器M的主频为1.5GHz，在M上执行程序P的指令条数为5×10<sup>5</sup>，P的平均CPI为1.2，则P在M上的指令执行速度和用户CPU时间分别为 (&emsp;)。
...
26\. 假定编译器对高级语言的某条语句可以编译生成两种不同的指令序列，A、B和C三类指令的CPI和两种不同序列所含的三类指令条数如下表所示，两个指令序列都在时钟周期为2ns的机器上运行，则下列结论中正确的是 (&emsp;)。

<img src="./06-计组基础.assets/1-25.png" style="zoom: 60%;" />
...
【考纲内容】<br/>
（一）数制与编码<br/>
&emsp;&emsp;&emsp;进位计数制及其相互转换；定点数的编码表示<br/>
（二）运算方法和运算电路<br/>
&emsp;&emsp;&emsp;基本运算部件：加法器；算术逻辑单元（ALU）<br/>
&emsp;&emsp;&emsp;加/减运算：补码加/减运算器；标志位的生成<br/>
&emsp;&emsp;&emsp;乘/除运算：乘/除法运算的基本原理；乘法电路和除法电路的基本结构<br/>
（三）整数的表示和运算<br/>
&emsp;&emsp;&emsp;无符号整数的表示和运算；有符号整数的表示和运算<br/>
（四）浮点数的表示和运算<br/>
&emsp;&emsp;&emsp;浮点数的表示：IEEE 754标准；浮点数的加/减运算
</syllabus>
...
移码的无符号真值 = 实际真值 + 偏置值。这里无符号真值和实际真值成正比。
:::
...
<options :options="['A. 0094H', 'B. FF94H', 'C. 0F94H', 'D. F094H']" />
...
::: answer
答案：(11110100)<sub>2</sub>，OF=0，CF=1。
:::
...
16\. (2023) 已知x,y为int型，当x=100,y=200时，执行“x减y”指令得到的溢出标志OF和借位标志CF分别为0,1，那么当x=10,y=-20时，执行该指令得到的OF和CF分别为 (&emsp;)。
...
32\. (2014) float型数据常用IEEE754单精度浮点格式表示。假设两个float型变量x和y分别存放在32位寄存器f1和f2中，若(f1)=CC90 0000H，(f2)=B0C0 0000H，则x和y之间的关系为 (&emsp;)。
...
<options :options="['A. -(2-2<sup>52</sup>)×2<sup>-1023</sup>', 'A. -(2-2<sup>52</sup>)×2<sup>+1023</sup>', 'C. -1×2<sup>-1024</sup>', 'D. -(1-2<sup>-52</sup>)×2<sup>+2047</sup>']" />
...
42\. 在C语言的不同类型的数据混合运算中，要先转换为同一类型后进行运算。设一表达式中包含有int型、long型、char型和double型的变量与数据，则表达式最后的运算结果是 (&emsp;)，这4种类型数据的转换规律是 (&emsp;)。
...
44\. (2010) 假定变量i、f和d的数据类型分别为int、float和double（int型用补码表示，float型和double型分别用IEEE754单精度和双精度浮点数格式表示），已知i=785，f=1.5678E3，d=1.5E100，若在32位机器中执行下列关系表达式，则结果为“真”的是 (&emsp;)。
...
（二）层次化存储器的基本结构<br/>
（三）半导体随机存储器（RAM）<br/>
&emsp;&emsp;&emsp;SRAM、DRAM、Flash存储器<br/>
（四）主存储器<br/>
&emsp;&emsp;&emsp;DRAM芯片和内存条、多模块存储器、主存储器和CPU之间的连接<br/>
（五）外部存储器<br/>
&emsp;&emsp;&emsp;磁盘存储器、固态硬盘（SSD）<br/>
（六）高速缓冲存储器（Cache）<br/>
&emsp;&emsp;&emsp;Cache的基本原理；Cache和主存之间的映射方式<br/>
&emsp;&emsp;&emsp;Cache中主存块的替换算法；Cache写策略<br/>
（七）虚拟存储器<br/>
&emsp;&emsp;&emsp;虚拟存储器的基本概念<br/>
&emsp;&emsp;&emsp;页式虚拟存储器：基本原理、页表、地址转换、TLB（快表）<br/>
&emsp;&emsp;&emsp;段式虚拟存储器的基本原理；段页式虚拟存储器的基本原理
</syllabus>
...
<options :options="['A. 16片, 07FFFFFFH', 'B. 16片, 0F800000H', 'C. 32片, 07FFFFFFH', 'D. 32片, 0FFFFFFH']" width="95%" />
...
<options :options="['A. 00000H, 01000H', 'B. 38000H, 3FFFFH', 'C. 3800H, 3FFFFH', 'D. 0000H, 0100H']" width="90%" />
...
<options :options="['A. RAID0', 'B. RAID1', 'C. RAID2', 'D. RAID3']" />
...
<options :options="['D. 若K>1且K&ltM，则该Cache是M/K路组相联映射Cache']" />
...
<options :options="['A. 52号和102号单元', 'B. 48号和308号单元', 'C. 60号和160号单元', 'D. 46号和236号单元']" width="95%" />
...
51\. (2021) 若计算机主存地址为32位，按字节编址，Cache数据区大小为32KB，主存块大小为32B，采用直接映射方式和回写法(Write Back)，则Cache行的位数至少是 (&emsp;)。
...
52\. 对于n路组相联映射Cache，在保持n及主存和Cache总容量不变的前提下，将主存块大小和Cache块大小都增加一倍，则下列描述中正确的是 (&emsp;)。
...
53\. 假设主存地址位数为32位，按字节编址，主存和Cache之间采用全相联映射方式，主存块大小为1个字，每字32位，采用回写法(Write Back)方式和随机替换策略，则能存放32K字数据的Cache的总容量至少应有 (&emsp;) 位。
...
54\. 若计算机按字编址，Cache数据区容量为8K字，主存块大小为512字，主存地址空间为1M字，采用2路组相联映射方式。每次根据主存地址访问Cache时，需要同时进行 (&emsp;) 次标记位的比较，每次需要比较的位数是 (&emsp;)。
...
<img src="./06-计组基础.assets/3-2011-1.png" style="zoom: 60%;" />
...
<img src="./06-计组基础.assets/3-2011-2.png" style="zoom: 60%;" />
...
<img src="./06-计组基础.assets/3-2016.png" style="zoom: 60%;" />
...
（三）寻址方式<br/>
（四）数据的对齐和大/小端存放方式<br/>
（五）CISC 和 RISC的基本概念<br/>
（六）高级语言程序与机器级代码之间的对应<br/>
&emsp;&emsp;&emsp;编译器、汇编器与链接器的基本概念；选择结构语句的机器级表示<br/>
&emsp;&emsp;&emsp;循环结构语句的机器级表示；过程（函数）调用对应的机器级表示
</syllabus>
...
<img src="./06-计组基础.assets/4-2016.png" style="zoom: 60%;" />
...
<img src="./06-计组基础.assets/4-25.png" style="zoom: 40%;" />
...
（四）控制器的功能和工作原理<br/>
（五）异常和中断机制<br/>
&emsp;&emsp;&emsp;异常和中断的基本概念；异常和中断的分类；异常和中断的检测与响应<br/>
（六）指令流水线<br/>
&emsp;&emsp;&emsp;指令流水线的基本概念；指令流水线的基本实现；<br/>
&emsp;&emsp;&emsp;结构冒险、数据冒险和控制冒险的处理；超标量和动态流水线的基本概念<br/>
（七）多处理器基本概念<br/>
&emsp;&emsp;&emsp;SISD、SIMD、MIMD、向量处理器的基本概念；硬件多线程的基本概念；<br/>
&emsp;&emsp;&emsp;多核（multi-core）处理器的基本概念；共享内存多处理器（SMP）的基本概念
</syllabus>
...
<options :options="['Ⅱ. 若采用微程序控制方式，则可用μPC取代PC']" />
...
【考纲内容】<br/>
（一）I/O接口（I/O控制器）<br/>
&emsp;&emsp;&emsp;I/O接口的功能和基本结构；I/O端口及其编址<br/>
（二）I/O方式<br/>
&emsp;&emsp;&emsp;程序查询方式<br/>
&emsp;&emsp;&emsp;程序中断方式：中断的基本概念；中断响应过程；中断处理过程；<br/>
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;多重中断和中断屏蔽的概念<br/>
&emsp;&emsp;&emsp;DMA方式：DMA控制器的组成；DMA传送过程<br/>
</syllabus>
