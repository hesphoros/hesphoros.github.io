...
（二）操作系统的发展历程<br/>
（三）程序运行环境<br/>
&emsp;&emsp;&emsp;CPU运行模式：内核模式与用户模式；中断和异常的处理；系统调用；<br/>
&emsp;&emsp;&emsp;程序的链接与装入；程序运行时内存映像与地址空间<br/>
（四）操作系统结构<br/>
&emsp;&emsp;&emsp;分层、模块化、宏内核、微内核、外核<br/>
（五）操作系统引导<br/>
（六）虚拟机
</syllabus>
...
<options :options="['D. MOV R0, addr ;把地址addr处的内存数据放入寄存器R0']" />
...
【考纲内容】<br/>
（一）进程与线程<br/>
&emsp;&emsp;&emsp;进程与线程的基本概念；进程/线程的状态与转换<br/>
&emsp;&emsp;&emsp;线程的实现：内核支持线程、线程库支持线程<br/>
&emsp;&emsp;&emsp;进程与线程的组织与控制<br/>
&emsp;&emsp;&emsp;进程间通信：共享内存、消息传递、管道、信号<br/>
（二）CPU调度与上下文切换<br/>
&emsp;&emsp;&emsp;调度的基本概念：调度的目标；<br/>
&emsp;&emsp;&emsp;调度的实现：调度器/调度程序（scheduler），调度的时机与调度方式（抢占式/非抢占式），闲逛进程，内核级线程与用户级线程调度<br/>
&emsp;&emsp;&emsp;CPU调度算法<br/>
&emsp;&emsp;&emsp;多处理机调度<br/>
&emsp;&emsp;&emsp;上下文及切换机制<br/>
（三）同步与互斥<br/>
&emsp;&emsp;&emsp;同步与互斥的基本概念<br/>
&emsp;&emsp;&emsp;基本的实现方法：软件方法；硬件方法<br/>
&emsp;&emsp;&emsp;锁：信号量；条件变量<br/>
&emsp;&emsp;&emsp;经典同步问题：生产者-消费者问题，读者-写者问题；哲学家进餐问题<br/>
（四）死锁<br/>
&emsp;&emsp;&emsp;死锁的基本概念：死锁预防<br/>
&emsp;&emsp;&emsp;死锁避免：死锁检测和解除
</syllabus>
...
<img src="./07-操作系统基础.assets/2-2018-1.png" style="zoom: 60%;" />
...
<img src="./07-操作系统基础.assets/2-2022.png" style="zoom: 60%;" />
...
<options :options="['Ⅳ. 时间片轮转调度算法']" /><br/>
...
<options :options="['A. 顺序代码', 'B. 机器语言代码', 'C. 不允许任何修改的代码', 'D. 无转移指令代码']" width="95%" />
...
<img src="./07-操作系统基础.assets/2-2018-2.png" style="zoom: 60%;" />
...
<img src="./07-操作系统基础.assets/2-103.png" style="zoom: 60%;" />
...
<img src="./07-操作系统基础.assets/2-2010-1.png" style="zoom: 60%;" />

若进程P0和P1访问临界资源的类C伪代码实现如下：

<img src="./07-操作系统基础.assets/2-2010-2.png" style="zoom: 60%;" />
...
111\. 有一个计数信号量S：<br/>
&nbsp;1\) 假如若干进程对S进行28次P操作和18次V操作后，信号量S的值为0。<br/>
...
<img src="./07-操作系统基础.assets/2-133.png" style="zoom: 60%;" />
...
<options :options="['A. 用PV操作管理共享资源', 'B. 使进程互斥地使用共享资源']" />
<options :options="['C. 采用资源静态分配策略', 'D. 定时运行死锁检测程序']" width="71%" />
...
<img src="./07-操作系统基础.assets/2-2011.png" style="zoom: 60%;" />
...
<img src="./07-操作系统基础.assets/2-2012.png" style="zoom: 60%;" />
...
<img src="./07-操作系统基础.assets/2-2022-150.png" style="zoom: 60%;" />
...
<img src="./07-操作系统基础.assets/2-2020.png" style="zoom: 60%;" />
...
【考纲内容】<br/>
（一）内存管理基础<br/>
&emsp;&emsp;&emsp;内存管理概念：逻辑地址与物理地址空间，地址变换，内存共享，内存保护，<br/>
&emsp;&emsp;&emsp;内存分配与回收<br/>
&emsp;&emsp;&emsp;连续分配管理方式：页式管理；段式管理；段页式管理<br/>
（二）虚拟内存管理<br/>
&emsp;&emsp;&emsp;虚拟内存基本概念：请求页式管理；页框分配与回收；页置换算法<br/>
&emsp;&emsp;&emsp;内存映射文件（Memory-Mapped Files）；虚拟存储器性能的影响因素及改进方式
</syllabus>
...
<img src="./07-操作系统基础.assets/3-13.png" style="zoom: 60%;" />
...
<options :options="['Ⅰ. 若关闭TLB，则每存取一条指令或一个操作数都至少要访存2次']" />
<options :options="['Ⅱ. 页式存储管理不会产生内部碎片']" />
<options :options="['Ⅲ. 页式存储管理中的页面是用户所能感知的']" />
<options :options="['Ⅳ. 页式存储方式可以采用静态重定位']" /><br/>
...
<options :options="['A. 1MB', 'B. 401MB', 'C. 1MB+2<sup>32</sup>MB', 'D. 2<sup>32</sup>B']" />
...
【考纲内容】<br/>
（一）文件<br/>
&emsp;&emsp;&emsp;文件的基本概念；文件元数据和索引节点（inode）<br/>
&emsp;&emsp;&emsp;文件的操作：建立、删除、打开、关闭、读、写<br/>
&emsp;&emsp;&emsp;文件的保护；文件的逻辑结构；文件的物理结构<br/>
（二）目录<br/>
&emsp;&emsp;&emsp;目录的基本概念：树形目录；目录的操作；硬链接和软链接<br/>
（三）文件系统<br/>
&emsp;&emsp;&emsp;文件系统的全局结构（layout）：文件系统在外存中的结构，文件系统在内存中的结构<br/>
&emsp;&emsp;&emsp;外存空间管理方法；虚拟文件系统；文件系统挂载（mounting）
</syllabus>
...
<options :options="['A. 磁盘引导块(boot block on disk)', 'B. 系统程序', 'C. OS内核', 'D. BIOS中的程序']" />
...
【考纲内容】<br/>
（一）输入/输出（I/O）管理基础<br/>
&emsp;&emsp;&emsp;设备：设备的基本概念，设备的分类，I/O接口<br/>
&emsp;&emsp;&emsp;I/O控制方式：轮询方式，中断方式，DMA方式<br/>
&emsp;&emsp;&emsp;I/O软件层次结构：中断处理程序，驱动程序，设备独立性软件，用户层I/O软件<br/>
&emsp;&emsp;&emsp;输入/输出应用程序接口：字符设备接口，块设备接口，网络设备接口，阻塞/非阻塞I/O<br/>
（二）设备独立性软件<br/>
&emsp;&emsp;&emsp;缓冲区管理；设备分配与回收；假脱机技术（SPOOLing）；设备驱动程序接口<br/>
（三）外存管理<br/>
&emsp;&emsp;&emsp;磁盘：磁盘结构，格式化，分区，磁盘调度算法<br/>
&emsp;&emsp;&emsp;固态硬盘：读写性能较好，磨损均衡
</syllabus>
...
<options :options="['A. 用户缓冲区', 'B. CPU中的通用寄存器']" width="70%" />
<options :options="['C. 内核缓冲区', 'D. 键盘控制器的数据寄存器']" />
...
67\. (2025) 在下列选项中，文件系统需要为温彻斯特硬盘和固态硬盘都提供的功能是 (&emsp;)。
